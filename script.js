...
function renderizarTarefas(lista, tarefas) {
    ...
    tarefas.forEach((tarefa, index) => {
        ...
        detalhes.innerHTML = `
            ...
            <p><strong>Criado em:</strong> ${formatarDataBR(tarefa.dataCriacao)}</p>
        `;
...
function limparFormulario() {
  ...
  document.getElementById('dataCriacao').value = dataISO; // yyyy-mm-dd
  tarefaEditando = null;
}
...
function mostrarSecao(secaoId) {
  ...
  // Se estiver indo para a tela de adicionar e não está editando, define a data de criação formatada
  if (secaoId === 'adicionar' && tarefaEditando === null) {
      const dataISO = new Date().toISOString().split('T')[0]; // yyyy-mm-dd
